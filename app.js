...
window.calcularPreco = function() {
    ...
    const resCartao = document.getElementById('res-cartao');
    const resPix    = document.getElementById('res-pix');
    if (resCartao) resCartao.innerText = `R$ ${vCartao.toFixed(2)}`;
    if (resPix)    resPix.innerText    = `R$ ${vPix.toFixed(2)}`;
...
window.salvarProduto = async function() {
    ...
    const estoque= parseInt(document.getElementById('p-qtd')?.value)            || 0; // ✅ p-qtd
    const custo  = parseFloat(document.getElementById('p-custo')?.value)        || 0;
    const frete  = parseFloat(document.getElementById('p-frete')?.value)        || 0;
    const desloc = parseFloat(document.getElementById('p-deslocamento')?.value) || 0;
    const margem = parseFloat(document.getElementById('p-lucro')?.value)        || 0;

    if (!nome)        return alert("⚠️ Informe o nome do produto!");
    if (estoque <= 0) return alert("⚠️ Informe a quantidade em estoque!");
    if (custo   <= 0) return alert("⚠️ Informe o custo unitário!");
    ...
    try {
        await addDoc(collection(db, "produtos"), {
            ...
            deslocamento:  desloc,
            margem,
            custoBase:     parseFloat(totalCusto.toFixed(2)),
            custoUnitario: parseFloat(custo.toFixed(2)),
            vendaPix:      parseFloat(vPix.toFixed(2)),
            vendaCartao:   parseFloat(vCartao.toFixed(2))
        });

        alert("✅ Produto cadastrado com sucesso!");

        // ✅ Limpa campos e volta valores padrão
        document.getElementById('p-nome').value         = "";
        document.getElementById('p-qtd').value          = "1";  // ✅ p-qtd
        document.getElementById('p-custo').value        = "";
        document.getElementById('p-frete').value        = "30";
        document.getElementById('p-deslocamento').value = "15";
        document.getElementById('p-lucro').value        = "40";
        ...
        const resCartao = document.getElementById('res-cartao');
        const resPix    = document.getElementById('res-pix');
        if (resCartao) resCartao.innerText = "R$ 0,00";
        if (resPix)    resPix.innerText    = "R$ 0,00";
...
window.fecharModalEditarProduto = function() {
    const m = document.getElementById('modal-editar-produto');
...
window.toggleTaxaCartao = function(pagamento) {
    ...
    window.autoPreencherVenda();
    if (carrinhoItens.length > 0) window.aplicarDesconto();
}
...
window.aplicarDesconto = function() {
    const descTipo  = document.getElementById('v-desc-tipo')?.value  || 'reais';
    const descValor = parseFloat(document.getElementById('v-desconto')?.value) || 0;
    ...
    let descReais = 0;
    if (descTipo === 'reais') {
        descReais = Math.min(descValor, totalBruto);
    } else {
        descReais = parseFloat(((descValor / 100) * totalBruto).toFixed(2));
    }

    const totalComDesconto = parseFloat((totalBruto - descReais).toFixed(2));

    const divDesc = document.getElementById('resumo-desconto');
    const spanOri = document.getElementById('desc-valor-original');
    const spanDes = document.getElementById('desc-valor-desconto');
    const spanFin = document.getElementById('desc-valor-final');

    if (divDesc) divDesc.style.display = descReais > 0 ? 'block' : 'none';
    if (spanOri) spanOri.innerText = `R$ ${totalBruto.toFixed(2)}`;
    if (spanDes) spanDes.innerText = `-R$ ${descReais.toFixed(2)}`;
    if (spanFin) spanFin.innerText = `R$ ${totalComDesconto.toFixed(2)}`;

    window.atualizarResumoPagamento(totalComDesconto);
}
    ...
    carrinhoItens.forEach((item, i) => {
        lista.innerHTML += `
            <div style="display:flex; justify-content:space-between;
                ...
                background:#111; border-radius:8px; margin-bottom:6px;
        ...
        // ✅ Mostra campo de desconto quando tem itens
        const divDesc2 = document.getElementById('div-desconto');
        if (divDesc2) divDesc2.style.display = carrinhoItens.length > 0 ? 'block' : 'none';
    

    if (divTot)  divTot.style.display = 'block';
    if (spanTot) spanTot.innerText    = `R$ ${total.toFixed(2)}`;

    // ✅ Aplica desconto (que por sua vez chama atualizarResumoPagamento)
    window.aplicarDesconto();
}
...
window.salvarVenda = async function() {
    ...
    try {
        ...
        const descTipo     = document.getElementById('v-desc-tipo')?.value || 'reais';
        const descValorInp = parseFloat(document.getElementById('v-desconto')?.value) || 0;
        const descReais    = descTipo === 'reais'
            ? Math.min(descValorInp, totalBruto)
            : parseFloat(((descValorInp / 100) * totalBruto).toFixed(2));
        const valorCliente = parseFloat((totalBruto - descReais).toFixed(2));
        ...
        const venda = {
            ...
            desconto:        parseFloat(descReais.toFixed(2)),
            pagamento:       pag,
        ...
        mensagemWhats += `━━━━━━━━━━━━━━━━━━━━\n`;
        if (descReais > 0) {
            mensagemWhats += `🏷️ Subtotal: R$ ${totalBruto.toFixed(2)}\n`;
            mensagemWhats += `🏷️ Desconto: -R$ ${descReais.toFixed(2)}\n`;
        }
        ...
        if (!clientesSnap.empty) {
            ...
            if ((contato && contato !== dadosAtuais.contato) ||
                (endereco && endereco !== dadosAtuais.endereco)) {
                const ok = confirm(
                ...
                if (ok) await updateDoc(doc(db, "clientes", clienteDoc.id), { contato, endereco });
            }
        } else {
        ...
        // ✅ Modal WhatsApp — funciona no celular
        const tel = contato.replace(/\D/g, '');
        const url = `https://wa.me/55${tel}?text=${encodeURIComponent(mensagemWhats)}`;

        const modalWhats = document.createElement('div');
        modalWhats.id = 'modal-whats';
        modalWhats.style.cssText = `
            position:fixed; top:0; left:0; width:100%; height:100%;
            background:rgba(0,0,0,0.88); z-index:99999;
            display:flex; align-items:center; justify-content:center;
            padding:20px; box-sizing:border-box;`;

        modalWhats.innerHTML = `
            <div style="background:#1a1a1a; border:1px solid #00ff00;
                border-radius:16px; padding:30px; width:100%;
                max-width:380px; text-align:center;">
                <div style="font-size:40px; margin-bottom:10px;">✅</div>
                <h3 style="color:#00ff00; margin:0 0 8px 0;">Venda Finalizada!</h3>
                <p style="color:#aaa; font-size:13px; margin:0 0 24px 0;">
                    Deseja enviar o comprovante pelo WhatsApp?
                </p>
                <div style="display:flex; flex-direction:column; gap:12px;">
                    ${contato ? `
                    <a href="${url}" target="_blank" rel="noopener noreferrer"
                        onclick="document.getElementById('modal-whats').remove(); setTimeout(()=>location.reload(),300);"
                        style="display:block; background:#25D366; color:white;
                        font-weight:bold; padding:14px; border-radius:10px;
                        text-decoration:none; font-size:15px;">
                        📱 SIM, ENVIAR WHATSAPP
                    </a>` : ''}
                    <button onclick="document.getElementById('modal-whats').remove(); setTimeout(()=>location.reload(),300);"
        ...
        document.body.appendChild(modalWhats);
...
window.estornarPagamentoFiado = async function(id) {
    ...
    await updateDoc(ref, {
        ...
        parcelasPagas: historico.length,
        historicoParc: historico,
        saldoRestante: novoSaldo
    });

    alert(`✅ Pagamento estornado!\n\n⚠️ Saldo em aberto: R$ ${novoSaldo.toFixed(2)}`);
    window.carregarFiados();
}
...
window.gerarRelatorio = async function() {
    try {
        ...
        snapVendas.forEach(d => {
            ...
            // ✅ FIADO PENDENTE GERAL
            if (v.status === "Pendente") {
                const historico    = v.historicoParc || [];
                const totalPagoDoc = historico.reduce(
                    (acc, h) => acc + parseFloat(h.valorPago || 0), 0
                );
                const saldoAberto = parseFloat((vLiquido - totalPagoDoc).toFixed(2));
                if (saldoAberto > 0) totalPendenteGeral += saldoAberto;
            }
            ...
            // ✅ VENDAS PAGAS — pelo data da venda
            if (v.status === "Pago") {
                if (dataVenda < inicio || dataVenda > fim) return;
                ...
                else if (pag === "Fiado")   { totalFiadoPago     += vLiquido; }
                ...
                prodContagem[v.perfume || "Desconhecido"] =
                    (prodContagem[v.perfume || "Desconhecido"] || 0) + 1;
            ...
            // ✅ FIADO PENDENTE — pela data das PARCELAS
            } else if (v.status === "Pendente" && pag === "Fiado") {

                const historico     = v.historicoParc  || [];
                const datasParcelas = v.datasParcelas   || [];
                const totalParcelas = v.parcelasTotais  || 1;
                const vParcela      = parseFloat((vLiquido / totalParcelas).toFixed(2));
                ...
                const parcNoPeriodo = datasParcelas.filter(
                    dp => dp >= inicio && dp <= fim
                );
                if (parcNoPeriodo.length === 0) return;

                const valorNoPeriodo = parseFloat(
                    (parcNoPeriodo.length * vParcela).toFixed(2)
                );

                qtdVendas++;
                totalGeral += valorNoPeriodo;

                if (totalPagoDoc > 0) {
                    totalFiadoPago += Math.min(totalPagoDoc, valorNoPeriodo);
                }

                if (custoVenda > 0) {
                    const proporcao  = valorNoPeriodo / vLiquido;
                    const lucroVenda = valorNoPeriodo
                        - (custoVenda    * proporcao)
                        - (freteDespesa  * proporcao)
                        - (deslocDespesa * proporcao);
                    totalLucro         += lucroVenda;
                    totalCustos        += custoVenda    * proporcao;
                    totalFreteDespesa  += freteDespesa  * proporcao;
                    totalDeslocDespesa += deslocDespesa * proporcao;
                }
                ...
                vendasPeriodo.push({
                    ...v,
                    _vLiquido:    valorNoPeriodo,
                    _vCliente:    vCliente,
                    _statusLabel: "Fiado Pendente",
                    _saldoAberto: saldoAberto,
                    _totalPago:   totalPagoDoc,
                    _parcPeriodo: parcNoPeriodo.length,
                    _datasParc:   parcNoPeriodo
                });
        ...
        const totalGeralComPendente = totalGeral + totalPendenteGeral;
        ...
        set('rel-fiado',         `R$ ${totalFiadoPago.toFixed(2)}`);
        set('rel-pendente',      `R$ ${totalPendenteGeral.toFixed(2)}`);
        ...
        set('rel-total-geral',   `R$ ${totalGeralComPendente.toFixed(2)}`);
        ...
        listaEl.innerHTML = vendasPeriodo.map(v => {
            ...
            const isPend = v._statusLabel === "Fiado Pendente";
            const cor    = pag === "PIX"      ? "#00ff00" :
                           pag === "Cartão"   ? "#ff4d4d" :
                           pag === "Dinheiro" ? "#4da6ff" : "var(--gold)";

            // ✅ Datas das parcelas do período
            const datasParc = v._datasParc || [];
            ...
            return `
                <div style="display:flex; justify-content:space-between;
                    ...
                    <div style="flex:1;">
                        ...
                        <small style="color:#aaa;">👤 ${v.cliente || "—"}</small><br>
                        <small style="color:#666;">
                            📅 <span style="color:#aaa;">Venda:</span> ${v.data || "—"}
                            ${isPend && datasParc.length > 0 ? `
                                &nbsp;|&nbsp;
                                <span style="color:orange;">📅 Parcela:</span>
                        ...
                        ${isPend ? `
                            <small style="color:orange;">
                                ⚠️ Pago: R$ ${(v._totalPago || 0).toFixed(2)} |
                                Em aberto: R$ ${(v._saldoAberto || 0).toFixed(2)}
                            </small>` : ''}
                    </div>
                    <div style="text-align:right; min-width:120px;">
                        ${isC ? `
                            <div style="font-size:10px; color:#aaa;">
                                Cliente: R$ ${vC.toFixed(2)}
                            </div>
                            <div style="font-size:10px; color:#ff4d4d;">
                                Taxa: -R$ ${tx.toFixed(2)}
                            </div>
                        ` : ''}
                        ...
                        <small style="color:${cor}; font-weight:bold;">${pag}</small><br>
                        <small style="color:${isPend ? 'orange' : '#00ff00'}; font-size:10px;">
                            ${isPend ? '⏳ PENDENTE' : '✅ PAGO'}
                        </small>
                    </div>
                </div>`;
        }).join("");
